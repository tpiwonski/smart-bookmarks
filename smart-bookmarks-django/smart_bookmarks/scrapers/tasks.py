...
@shared_task(bind=True, base=ScraperTask)
def scrape_page(self, task_id):
    self.scraper_service.process_scrape_page_task(task_id=task_id)


@shared_task(bind=True)
def scrape_pages(self):
    tasks = ScrapePageTask.objects.tasks_to_process()
    for task in tasks:
        scrape_page.delay(task_id=task.id)
