...
from smart_bookmarks.scrapers.models import ScrapePageError, ScrapePageTask
from smart_bookmarks.scrapers.selenium import ScrapeError, SeleniumScrapePageService
...
class ScrapePageService(ScrapePageInterface):
    ...
    def scrape_page_async(self, bookmark):
        ...
        LOGGER.info(
            "Scrape page created: task_id=%s, bookmark_id=%s", task.id, bookmark.id
        )
    ...
    def scrape_page(self, bookmark):
        ...
        page_data = PageData(
            ...
            else scraped_data.text[:255].title,
            description=scraped_data.description[:1024]
            if scraped_data.description
            else scraped_data.text[:1024],
            text=scraped_data.text,
            source=scraped_data.source,
        )
        page = self.page_service.create_page(bookmark, page_data)
        LOGGER.info(
            "Page scraped for bookmark: page_id=%s, bookmark_id=%s",
            page.id,
            bookmark.id,
        )
        return page
