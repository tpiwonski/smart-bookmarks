...
class SearchBookmarkService(SearchBookmarkInterface):
    ...
    def search(self, query: str, operator: str, per_page: int):
        return SearchQuerySet(self, SearchQuery(query, operator), per_page)
...
class SearchQuerySet:
    def __init__(
        self, service: SearchBookmarkService, query: SearchQuery, per_page: int
    ):
    ...
    def load(self, offset, length):
        ...
        results, total_hits = self.service.search_q(
            query=self.query, offset=offset, length=length,
        )

        self.total_hits = total_hits
        self.cached_results[(offset, len(results))] = results
...
def query_results_loader(querySet: SearchQuerySet) -> QueryResults:
    pass
